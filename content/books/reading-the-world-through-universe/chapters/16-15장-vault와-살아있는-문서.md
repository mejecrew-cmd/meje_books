...
가설 항목은 아직 공개되거나 확정되지 않은 가능성이다. 창작자는 가설을 필요로 하는데, 모든 것을 처음부터 정본으로 만들 수는 없기 때문이다. 그러나 가설이 정본처럼 사용되면 혼란이 생기므로, 특히 협업자가 많을수록 가설 상태를 명확히 해야 한다. 어떤 설정은 시험 중이고, 어떤 설정은 다음 시즌 후보이며, 어떤 설정은 팬 반응을 보고 판단해야 한다고 표시되어야 한다.

팬덤 항목은 팬이 만든 해석, 밈, 관습, 커뮤니티 기억을 기록한다. 이것은 공식 캐논과 다르지만, 팬덤이 실제로 세계를 어떻게 읽는지 보여 주기 때문에 매우 중요하다. 팬덤 항목은 공식 정본으로 바로 올라가지 않더라도 Vault에 들어올 가치가 있다. 팬덤의 언어는 세계관의 사용 기록이다.

위험 항목도 필요하다. 루머, 과잉 해석, 사생활 침해, 혐오 표현, 반복되는 오해, 커뮤니티 갈등은 공개 문서에 넣을 내용은 아닐 수 있다. 그러나 운영자는 알고 있어야 하고, 위험 항목을 기록해야 대응할 수 있다. 살아있는 Vault는 아름다운 설정만 담지 않고, 세계를 해칠 수 있는 신호도 함께 기록한다.
...
맥락도 중요해서, 같은 키워드라도 출처와 맥락에 따라 의미가 달라진다. “구독경제”가 일반 SaaS 모델에서 쓰일 때와 팬덤 멤버십에서 쓰일 때가 다르고, “세계관”이 판타지 설정에서 쓰일 때와 리얼 IP의 팬덤 커뮤니티에서 쓰일 때도 다르다. Vault는 표제어의 정의만이 아니라 사용 맥락을 저장해야, 용어가 죽은 사전 항목이 되지 않는다.
...
정합성은 단순히 설정 오류를 피하는 일이 아니라, 팬덤 커뮤니티에서는 신뢰의 문제이다. 팬은 세계가 자신이 배운 규칙을 존중한다고 느낄 때 계속 머문다. 물론 모든 것을 고정할 필요는 없고 세계는 변할 수 있지만, 그 변화가 세계 안에서 납득 가능해야 한다. Vault는 무엇이 고정이고 무엇이 변주 가능한지 알려 준다.
...
정합성 검증은 창작을 억압하는 장치가 아니라, 오히려 창작의 자유를 보호한다. 무엇이 핵심인지 알면 무엇을 바꿀 수 있는지도 알 수 있다. Vault는 금지 목록이 아니라 변주 가능성의 지도이다. 세계가 어떤 부분에서 유연하고 어떤 부분에서 단단한지 알 때 협업자는 더 과감하게 만들 수 있다.
...
예를 들어 팬이 특정 소품에 강하게 반응했다면 그것은 굿즈 기회일 수 있지만, 동시에 그 소품이 세계의 어떤 의미와 연결되는지 확인해야 한다. 단순히 반응이 좋다고 상품화하면 팬은 얄팍하게 느낄 수 있다. Vault는 팬 반응과 세계관 의미를 연결해 주어, 팬이 좋아한 것이 무엇인지, 왜 좋아했는지, 어디까지 확장해도 되는지 판단하게 한다.
...
이렇게 보면 굿즈는 단순 상품이 아니라, 세계관 지식체계와 팬덤 행동이 만나는 노드이다. Vault는 이 노드의 관계를 기록한다. 좋은 IP 운영은 물건을 팔고 끝내지 않고, 물건이 세계 안에서 어떤 기억과 역할을 얻었는지 다시 읽는다.
...
Vault가 살아있으려면 운영 원칙이 필요하다. 첫째, 항목에는 상태가 있어서 정본, 가설, 팬덤 지식, 위험 신호, 폐기 항목이 구분되어야 한다. 둘째, 항목에는 출처가 있어서 공식, 내부, 팬덤, 산업 자료, 강연, 메일링, AI 생성물이 구분되어야 한다. 셋째, 항목에는 연결이 있어야 하는데, 고립된 항목은 지식체계가 아니라 메모이기 때문이다.

넷째, 항목은 갱신 기록을 가져서 언제 어떤 이유로 수정되었는지 남겨야 한다. 세계관은 변할 수 있지만, 변한 이유가 사라지면 나중에 같은 혼란이 반복된다. 다섯째, 항목은 사용처를 가져야 한다. 이 표제어가 LOREBOOK, 캐릭터빌드, 스토리텔링100, 팬덤 액티비티, 커머스, 라이선스, 커뮤니티 가이드 중 어디에 쓰이는지 알아야 한다.

여섯째, Vault는 과도하게 비대해지지 않도록 관리해야 한다. 모든 생각을 표제어로 만들면 사용하기 어려우므로, 반복 관찰과 관계가 있는 항목만 노드가 되어야 한다. 일회성 메모는 메모로 남기고, 지식체계의 노드는 기준을 통과해야 한다. 살아있는 문서는 계속 늘어나는 문서가 아니라 계속 정리되는 문서이다.
...
Vault를 정본의 저장소로만 보는 것도 오해로, Vault에는 정본뿐 아니라 가설, 팬덤 지식, 위험 신호, 폐기 항목도 필요하다. 정본만 모으면 세계관의 깨끗한 얼굴은 보이지만 실제 작동은 보이지 않는다. 팬덤 커뮤니티를 다루는 IP에서는 비공식 지식과 위험 신호를 관찰해야 한다.
...
Vault는 세계관의 장기 기억이어서, 사람이 바뀌고 플랫폼이 바뀌고 프로젝트가 늘어나도 세계의 판단 근거가 남아 있어야 한다. 팬덤 커뮤니티는 하루아침에 만들어지지 않고, 오래된 기억과 반복된 행동이 쌓여 만들어진다. Vault는 그 축적을 잃지 않게 하는 장치이다.
...
IP가 팬덤 커뮤니티가 되려면 세계관은 설정집으로 고정되어서는 안 되고, 팬 반응과 산업 변화와 창작 결과가 다시 들어와 갱신되어야 한다. Vault는 그 갱신의 중심이다. 살아있는 문서는 매번 완성되는 것이 아니라, 세계가 사용될 때마다 더 정확해지는 문서이다. 다음 장에서는 그 지식체계를 여럿이 읽고 쓸 수 있게 만드는 LOREBOOK으로 넘어간다.
